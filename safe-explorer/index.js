...
<body>
  ...
  <div class="level-card level-1" onclick="startSession(1)">
    <h3>🌐 Level 1: Surface</h3><p>Safe web</p><strong>👶 Beginner</strong>
  </div>
  <div class="level-card level-2" onclick="startSession(2)">
    <h3>🔍 Level 2: Deep Web</h3><p>Academic DBs</p><strong>📚 Educational</strong>
  </div>
  <div class="level-card level-3" onclick="startSession(3)">
    <h3>🕵️ Level 3: Dark Web</h3><p>Tor preview</p><strong>⚠️ Advanced</strong>
  </div>
  ...
  <script>
    ...
    function loadLevel() {
      ...
      if (currentLevel === 1) {
        status.innerHTML = '🌐 Level 1 Active';
        frame.src = 'https://duckduckgo.com/?q=safe+browsing';
      } else if (currentLevel === 2) {
        status.innerHTML = '🔍 Level 2 Active';
        frame.src = 'https://scholar.google.com/scholar?q=deep+web';
      } else {
        status.innerHTML = '🕵️ Level 3 Active';
        frame.src = 'https://www.torproject.org/';
      }
    }
    function startTimer() {
      let timeLeft = 1800;
